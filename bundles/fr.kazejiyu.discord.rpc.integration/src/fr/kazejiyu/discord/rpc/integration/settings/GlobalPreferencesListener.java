...
public class GlobalPreferencesListener implements IPropertyChangeListener {
    ...
    public GlobalPreferencesListener(Collection<SettingChangeListener> listeners) {
        ...
        events.put(USE_CUSTOM_APP.property(), (event, listener) -> listener.customDiscordApplicationVisibilityChanged(parseBoolean((String) event.getNewValue())));
        events.put(CUSTOM_APP_ID.property(), (event, listener) -> listener.discordApplicationIdChanged((String) event.getNewValue()));events.put(USE_CUSTOM_WORDING.property(), (event, listener) -> listener.useCustomWording(parseBoolean((String) event.getNewValue())));
